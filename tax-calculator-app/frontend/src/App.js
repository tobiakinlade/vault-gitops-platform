  ...
  return (
    <div className="App">
      <header className="App-header">
        ...
        <p className="subtitle">HMRC-Style Tax Calculation Demo with HashiCorp Vault</p>
        {health && (
          ...
          <form onSubmit={calculateTax}>
            <div className="form-group">
              <label htmlFor="income">
                ...
                <span className="tooltip">ℹ️
                  <span className="tooltiptext">Enter your gross annual salary</span>
                </span>
              </label>
              <input
                id="income"
            ...
            <div className="form-group">
              <label htmlFor="ni">
                ...
                <span className="tooltip">ℹ️
                  <span className="tooltiptext">Encrypted with Vault Transit Engine</span>
                </span>
              </label>
              <input
                id="ni"
            ...
            <div className="result-card">
              ...
              <div className="result-grid">
                ...
                <div className="result-item highlight">
                  ...
                  <span className="value negative">-{formatCurrency(result.income_tax)}</span>
                </div>
                <div className="result-item highlight">
                  <span className="label">National Insurance:</span>
                  <span className="value negative">-{formatCurrency(result.national_insurance_contribution)}</span>
                </div>
                <div className="result-item total">
                  <span className="label">Take Home Pay:</span>
                  <span className="value positive">{formatCurrency(result.take_home)}</span>
                </div>
                <div className="result-item">
                  <span className="label">Effective Tax Rate:</span>
                  <span className="value">{result.effective_rate.toFixed(2)}%</span>
                </div>
              </div>
              <div className="encrypted-info">
                <h4>🔐 Security Information</h4>
                <p><strong>Calculation ID:</strong> {result.id}</p>
                <p><strong>Encrypted NI:</strong> <code>{result.encrypted_ni.substring(0, 40)}...</code></p>
                <small>National Insurance number is encrypted using Vault Transit Engine before storage</small>
              </div>
            </div>
          )}
        </div>
              ...
              {history.map((item, index) => (
                <div key={item.id || index} className="history-item">
                  <div className="history-header">
                    <span className="history-income">{formatCurrency(item.income)}</span>
                    <span className="history-date">{formatDate(item.timestamp)}</span>
                  </div>
                  <div className="history-details">
                    <span>Tax: {formatCurrency(item.income_tax)}</span>
                    <span>NI: {formatCurrency(item.national_insurance_contribution)}</span>
      ...
      <footer className="App-footer">
        ...
        <p className="disclaimer">
          Demo application for interview purposes • Tax rates: UK 2024/2025
        </p>
      </footer>
    </div>
  );
